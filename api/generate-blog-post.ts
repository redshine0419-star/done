...
// Fallback recipes used by Vercel Cron (GET requests)
...
async function saveDraft(post: Record<string, unknown>, recipeId: string): Promise<Record<string, unknown>> {
...
export default async function handler(req: VReq, res: VRes) {
  ...
  // GET = Vercel Cron 자동 생성
  const recipe: Recipe = req.method === 'POST' && req.body?.recipe
    ? req.body.recipe
    : CRON_RECIPES[Math.floor(Math.random() * CRON_RECIPES.length)];

  try {
    const prompt = buildPrompt(recipe);
    const post = await callClaude(prompt);
    const saved = await saveDraft(post, recipe.id);
    res.json({ post: saved });
  } catch (err) {
